...
public final class LoadedBhone {
    ...
    private Type type;
    private Node tg = null;
    private Matrix4f tgTrans = null;
    ...
    private final Bhone getBhone(LoadedSkin s, Matrix4f x){
        if(bi == null){
            // this.bind(x, s);
            ...
            if(this.type == Type.Linear){                
                bi = new LinearBhone((AceleratedBhone)parent.bi, pod, points, pointsSrc, normals, normalsSrc, mov.getX(), mov.getY(), mov.getZ(), map);
            }else{
    ...
    public final Bhone getBhone(LoadedSkin s){
        return getBhone(s, new Matrix4f());
    }
