...
public final class SkinLoader {
	...
	//	j = 0;
		...
		for(int i = 0; i < vertices.length; i++){
			coords[i*3] = vertices[i].getX();
			coords[(i*3)+1] = vertices[i].getY();
			coords[(i*3)+2] = vertices[i].getZ();
		}
        // ar.setCoordRefFloat(coords);

		float texCoordsRef[] = new float[texCoords.length*2];
		for(int i = 0; i < texCoords.length; i++){
			texCoordsRef[i*2] = texCoords[i].getS();
			texCoordsRef[(i*2)+1] = texCoords[i].getT();
		}
		//ar.setTexCoordRefFloat(0, texCoordsRef);

		float normalRef[] = new float[normals.length*3];
		for(int i = 0; i < normals.length; i++){
			normalRef[i*3] = normals[i].getX();
			normalRef[i*3+1] = normals[i].getY();
			normalRef[i*3+2] = normals[i].getZ();
		} */
        ...
        //  {// save cache
        return new LoadedSkin(vertices, normals, texCoords, coordIndexes, normalIndexes, textureIndexes, vertexNormal);
    }
