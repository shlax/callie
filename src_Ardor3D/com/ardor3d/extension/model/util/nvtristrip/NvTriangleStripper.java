...
public class NvTriangleStripper implements Visitor {
    ...
    public void visit(final Spatial spatial) {
        if (spatial instanceof Mesh) {
            ...
            if (_reorderVertices) {
                ...
                reorderVertexData(newOrder.get(), mesh.getMeshData());
            }
            ...
            reorderIndices(newIndices, modes, counts, mesh.getMeshData());
        }
    }

    public void reorderIndices(final IndexBufferData<?> newIndices, final IndexMode[] modes, final int[] lengths, MeshData _meshData) {
    ...
    protected void reorderVertexData(final int[] newVertexOrder, final MeshData meshData) {
        ...
        for (int i = 0; i < meshData.getVertexCount(); i++) {
            ...
            copy(meshData.getVertexBuffer(), i * verts.getValuesPerTuple(), verts.getBuffer(), vert
                    * verts.getValuesPerTuple(), verts.getValuesPerTuple());
            if (norms != null) {
                copy(meshData.getNormalBuffer(), i * norms.getValuesPerTuple(), norms.getBuffer(), vert
                        * norms.getValuesPerTuple(), norms.getValuesPerTuple());
            }
            if (colors != null) {
                copy(meshData.getColorBuffer(), i * colors.getValuesPerTuple(), colors.getBuffer(), vert
                        * colors.getValuesPerTuple(), colors.getValuesPerTuple());
            }
            if (fogs != null) {
                copy(meshData.getFogBuffer(), i * fogs.getValuesPerTuple(), fogs.getBuffer(), vert
                        * fogs.getValuesPerTuple(), fogs.getValuesPerTuple());
            }
            if (tangents != null) {
                copy(meshData.getTangentBuffer(), i * tangents.getValuesPerTuple(), tangents.getBuffer(),
                        vert * tangents.getValuesPerTuple(), tangents.getValuesPerTuple());
            }
            for (int k = 0; k < uvs.length; k++) {
                if (uvs[k] != null) {
                    copy(meshData.getTextureBuffer(k), i * uvs[k].getValuesPerTuple(), uvs[k].getBuffer(),
