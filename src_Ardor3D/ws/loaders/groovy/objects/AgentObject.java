...
public final class AgentObject extends FactoryElement {
    ...
    public final AiWolker getAiWolker(NodeMapBuilder mapBuilder, Node aiNode, Node activeNode, Node efectNode, /*LsObject lsObject, */Float activeDistance){
        // AiWolker
    ...
    private void setUserData(Node g, Object val){
        for(Spatial nod : g.getChildren()){
            //Node nod = g.getChild(i);
            if(nod instanceof Mesh) nod.setUserData(val);
            else if(nod instanceof Node) setUserData((Node)nod, val);
        }
    }
