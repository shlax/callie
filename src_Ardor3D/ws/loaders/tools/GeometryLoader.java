...
import ws.loaders.tools.opt.MeshOpt;
...
public final class GeometryLoader {
   ...
   //     System.out.println('a');
	...
	//	String arr[] = tmp.split("\\}\\{");
        ...
        FloatBuffer vertexBuffer = BufferUtils.createVector3Buffer(coordIndexes.length);
        FloatBuffer normalBuffer = BufferUtils.createVector3Buffer(coordIndexes.length);
        FloatBuffer textureBuffer[] = new FloatBuffer[coordCount]; //;(coordIndexes.length/3);
        for(int k = 0;  k < coordCount; k++) textureBuffer[k] = BufferUtils.createVector2Buffer(coordIndexes.length);
        ...
        for (int i = 0; i < coordIndexes.length; i++){
            ...
            for(int k = 0;  k < coordCount; k++){
                ...
                textureBuffer[k].put(tmp.getX()).put(tmp.getY());
            }
            indexBuffer.put(i);
        }
        ...
        return new MeshOpt().getMeshData(vertices, normals, texCoords, coordIndexes, normalIndexes, textureIndexes);
    ...
    private final MeshData load(String f) throws IOException {
        ...
        for (int i = 0; i < coordIndexes.length; i++){
            ...
            for(int k = 0;  k < coordCount; k++){
                ...
                textureBuffer[k].put(t.getX()).put(t.getY());
            }
            indexBuffer.put(i);
        }
